...
<style>
    ...
    .book-card-hover{
        ...
        box-shodow: 0 10px 15px -3px rgba(0,0,0,0.1);
    }
...
def create_visualisation(stats):
    ...
    if stats['decades']:
        ...
        #load library
load_library()
st.sidebar.markdown("<h1 style='text-align: ceenter;'> Navigation</h1>", unsafe_allow_html=True)
...
if st.session_state.current_view == "add":
    ...
    with st.form(key='add_book_form'):
        ...
        with col1:
            tille = st.text_input("Book Title", max_chars=100)
        ...
        with col2:
            genre = st.selectbox("Genre",[
                "Friction", "Non-Friction", "Science", "Technology", "Romance", "Poetry", "Self-help", "Art", "Religion", "History", "Other"
            ])
            read_status = st.radio("Read Status", ["Read", "Unread"], horizontal=True)
            read_book = read_status == "Read"
        submit_button = st.form_submit_button(label="Add Book")

        if submit_button and tille and author:
            add_book(tille,author, publication_year,genre,read_book)
    ...
    if not st.session_state.library:
        st.markdown("<div class='warning-message'> Your library is empty. Add some book to get started!</div>", unsafe_allow_html=True)
    else:
        cols = st.columns(2)
...
""", unsafe_allow_html=True)
                ...
                with col2:
                    ...
                    status_lable = "Mark as read" if not book['read_status'] else "Mark as unread"
                    if st.button(status_lable, key=f"status_[i]", use_container_width=True):
                        st.session_state.library[i]['read_status'] = new_status
                        save_library()
                        st.rerun()
    if st.session_state.book_removed:
        st.markdown("<div class='sucess-message'> Book remove sucessfully!</div>", unsafe_allow_html=True)
        st.session_state.book_removed = False
...
st.markdown("---")
st.markdown("Copyright @ 2025 Sajid Sheikh Personal Library Manager", unsafe_allow_html=True)